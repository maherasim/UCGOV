...
function RespondModal({ newsletter, onClose }) {
    ...
    const mutation = useMutation({
        mutationFn: () =>
            client.post(`/api/adlg/newsletters/${newsletter.id}/respond`, {
                newsletter_option_id: optionId,
                remarks,
            }),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['adlg-newsletters'] });
            onClose();
        },
...
export default function Newsletters() {
    ...
    const { data, isLoading } = useQuery({
        queryKey: ['adlg-newsletters'],
        queryFn: () => client.get('/api/adlg/newsletters').then((r) => r.data),
    });
