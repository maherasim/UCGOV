...
function SubmitInquiryModal({ open, onClose }) {
    ...
    const mutation = useMutation({
        mutationFn: () => {
            ...
            return client.post('/api/ddlg/inquiries', formData);
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['ddlg-inquiries'] });
            close();
        },
...
export default function Inquiries() {
    ...
    const { data, isLoading } = useQuery({
        queryKey: ['ddlg-inquiries'],
        queryFn: () => client.get('/api/ddlg/inquiries').then((r) => r.data.data),
    });
    ...
    return (
        <div>
            ...
            <p className="mb-4 text-sm text-ink-muted">
                Every inquiry filed by an ADLG in your district, plus any you've filed yourself.
            </p>
                            ...
                            <div className="flex items-start justify-between">
                                <div>
                                    ...
                                    <div className="text-xs text-ink-muted">
                                        {i.ref} · {i.union_council || 'General'} · {i.adlg}
                                    </div>
                                </div>
