...
export default function Attendance() {
    ...
    const ucs = useQuery({
        queryKey: ['dg-union-councils'],
        queryFn: () => client.get('/api/dg/union-councils').then((r) => r.data.data),
    });

    const attendance = useQuery({
        queryKey: ['dg-attendance', appliedFilters],
        queryFn: () =>
            client
                .get('/api/dg/attendance', {
                    params: {
    ...
    const movement = useQuery({
        queryKey: ['dg-movement-log'],
        queryFn: () => client.get('/api/dg/movement-log').then((r) => r.data.data),
    ...
    const exportUrl = () => {
        ...
        return `${APP_BASE_PATH}/api/dg/attendance/analytics-export${qs ? `?${qs}` : ''}`;
    };
        ...
        <div>
            <div className="mb-4 flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-ink-muted">Every secretary across your division — view only</p>
                </div>
                    ...
                    {attendance.isLoading ? (
                        ...
                        <Card>
                            <DataTable
                                ...
                                slots={{
                                    5: (data) => <Badge tone={data === 'present' ? 'success' : 'warning'}>{data}</Badge>,
                                    6: (data) => <Badge tone={data ? 'success' : 'danger'}>{data ? 'Inside' : 'Outside'}</Badge>,
                                    7: (data) => <Badge tone={data ? 'success' : 'neutral'}>{data ? 'Verified' : 'Not verified'}</Badge>,
                                    8: (data, row) => (
                ...
                (movement.isLoading ? (
                    ...
                    <div>
                        <div className="mb-3 flex justify-end">
                            <Button variant="ghost" onClick={() => window.open(`${APP_BASE_PATH}/api/dg/movement-log/export`, '_blank')}>
                                📥 Export Excel
                            </Button>
                        </div>
                        <Card>
