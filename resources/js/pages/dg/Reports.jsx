...
export default function Reports() {
    ...
    const { data, isLoading } = useQuery({
        queryKey: ['dg-reports'],
        queryFn: () => client.get('/api/dg/reports').then((r) => r.data.data),
    });
        ...
        <div>
            <div className="mb-4 flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-ink-muted">Every report across your division — view only</p>
                </div>
                <Button variant="ghost" onClick={() => window.open(`${APP_BASE_PATH}/api/dg/reports/export`, '_blank')}>
                    📊 Export Excel
                </Button>
            </div>
            ...
            <Card>
                <DataTable
                    ...
                    slots={{
                        9: (data) => <Badge tone={data ? 'success' : 'warning'}>{data ? 'Reviewed' : 'Pending'}</Badge>,
                        10: (data, row) => (
